...
def get_normalized_vector(d):
    d /= (1e-12 + tf.reduce_max(tf.abs(d), [i for i in range(1, len(d.get_shape()))], keep_dims=True))
    d /= tf.sqrt(1e-6 + tf.reduce_sum(tf.pow(d, 2.0), [i for i in range(1, len(d.get_shape()))], keep_dims=True))
    return d
